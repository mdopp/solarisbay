...
CI/typecheck/tests verify *code* correctness, not *feature* correctness — and OSCAR's CI only **builds images**; it never exercises the assembled stack. The real ServiceBay box catches what the test harness can't. The gate is two-sided and you own the second:
...
## Steps
1. **Stage the merged code through ServiceBay.** OSCAR deploys *through* ServiceBay, so staging = install/refresh the affected artifact on the box via ServiceBay's install path (API/MCP). Confirm the `mdopp/oscar` registry is enabled in ServiceBay on the box so changed templates resolve.
...
3. **Verify.** Run `/verify` exercising the merged path-mandated changes (the `detail` from your context line names which). Per area:
   - Template/skill → pod becomes healthy; for skill changes `sudo ls /mnt/data/stacks/oscar-household/skills/` is populated, `podman exec hermes-hermes ls /opt/data/skills/oscar/` shows the skill, and Hermes' loader log lists it (no `TODO (rewrite)` stub if the issue was to finish it).
   - voice-gatekeeper → the Wyoming bridge connects with no `AsrModel.__init__()`-class crash; STT/TTS handoff to Hermes works.
   - database → the `oscar-household-init` sidecar runs `alembic upgrade head` cleanly; `oscar.db` schema is current.
...
5. **On verify red:** the change is already on `main`. First **triage the owner** (see Cross-repo routing): an OSCAR-owned regression → identify the culprit (a cluster keeps it attributable; an unrelated batch needs a bisect), open a **revert PR**, merge it on CI-green, re-run this verify; write `verify-result.json` `status:"red"` so the orchestrator holds the release. A **ServiceBay-platform** failure → do **not** revert a correct OSCAR change: file it upstream (`gh issue create --repo mdopp/servicebay`, symptom + servicebay file/line + the OSCAR repro), note it in the result `detail`, and write `status:"owed"` (the planner will mark the local issue blocked + `upstream_waits[]` next run; release stays held until the upstream fix lands and re-verify is green).
...
- **OSCAR-owned** — the bug is in `templates/**`, `templates/oscar-household/skills/**`, `voice-gatekeeper/src/gatekeeper/**`, `database/**`, `stacks/**`, or `plugin.yaml`/`__init__.py` → revert here / fix in the loop.
- **ServiceBay-owned (upstream)** — the bug is in the platform OSCAR depends on (install runner / asset-transport, the agent, MCP wiring, NPM/reverse-proxy, registry resolver, onboarding/portal). The OSCAR loop **cannot** fix ServiceBay from this repo → file an `mdopp/servicebay` issue (the handoff; don't open a cross-repo PR) and write `status:"owed"`.
- **Both** → split: revert/fix the OSCAR-side part, file the platform part upstream.

## Return
`Verify: green @ a1b2c3d on the box (templates/ + skills/); release cleared.` — or `…red, reverted OSCAR regression via PR #47, release blocked.` — or `…red, ServiceBay-owned, filed servicebay#1234, verify owed.` — or `…image not yet on box, verify owed (post-publish).`
...
- Revert a correct OSCAR change for a ServiceBay-platform failure — route it upstream instead.
