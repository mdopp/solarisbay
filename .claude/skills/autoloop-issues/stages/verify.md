...
3. **Verify.** Run `/verify` exercising the merged path-mandated changes (the `detail` from your context line names which). Per area:
   - Template/skill → pod becomes healthy; for skill changes `sudo ls /mnt/data/stacks/solarisbay/skills/` is populated, `podman exec hermes-hermes ls /opt/data/skills/solaris/` shows the skill, and Hermes' loader log lists it (no `TODO (rewrite)` stub if the issue was to finish it).
