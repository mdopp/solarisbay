"""Client for the Sol Engine's Ollama-compatible facade.

The engine (solilos-chat) exposes `/ollama/api/chat` — stateless, the caller
...
control default) runs on `sol` (the engine's fast household profile); an
explicit "think harder" cue (Gründlich) runs on `sol-deep` (12b, thinks).
...
class SolClient:
    ...
    async def converse(
        ...
        *,
        ...
        if model != FAST_MODEL:
            log.info("gatekeeper.sol.reasoning", trace_id=trace_id, effort=effort)
        ...
        try:
            ...
            log.error("gatekeeper.sol.unreachable", trace_id=trace_id, error=str(e))
            return ""
        if response.status_code >= 400:
            log.error(
                "gatekeeper.sol.error",
                trace_id=trace_id,
