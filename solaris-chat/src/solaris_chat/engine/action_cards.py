...
use (#702) — it runs only when the callback re-sends `confirmed=true`. The
registry keeps the policy (which ids exist, which are destructive) in one place;
the endpoint is a thin dispatcher over it.
"""
...
class ActionHandler:
    ...
    carry `confirmed=true`); `run` receives the callback body and returns the
    JSON result surfaced to the client.
    """
...
def register(action_id: str, handler: Handler, *, destructive: bool = False) -> None:
    """Register a handler for an action-card button id (used by #788/#790)."""
    _REGISTRY[action_id] = ActionHandler(run=handler, destructive=destructive)
