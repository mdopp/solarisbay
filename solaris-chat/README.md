# solaris-chat — the Solaris Engine
...
whisper STT) → `conversation.solaris` → this facade → wyoming piper TTS →
...
into the resident uid — no second login. `SOLARIS_API_KEY` (the facade bearer)
