...
| `cloud_audit` | Append-only — one row per cloud-LLM call: timestamp, uid, trace_id, vendor, model, prompt/response hash + length, cost, router score, escalation reason. Full text gated by `system_settings.debug_mode`. | 0 |
...
SQLite. Rationale: Solaris's tables are a few-rows-of-config job (tens of audit rows per day; one settings row; ten embedding rows). Phase 3a re-opens the storage choice when domain collections (`books`, `records`, `documents`, `audiobooks`, `experiences`) land. See [`../solaris-architecture.md`](../solaris-architecture.md) → "The schema".
