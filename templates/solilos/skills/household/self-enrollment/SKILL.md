...
# Solilos — Self-Enrollment (the spoken "Setup starten" entry point)

## Overview

This is the **self-service** voice-enrolment flow: a household-tier speaker asks
to set themselves up, and Sol walks them through enrolling a voice profile and
...
- For a `guest` turn — that is `sol-guest-onboarding` (#375) → `resident-registration`.
...
- `#375` / `sol-guest-onboarding` — the guest greeting + fork (the parallel path
  for heard-but-below-threshold speakers).
- `sol-resident-registration` — the guest-side registration flow this mirrors.
