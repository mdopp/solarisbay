...
Solaris can't resolve "the light" — so it asks once, remembers the answer, and
carries on.

The mapping lives in `solaris.db` and is written through the **`set_room` MCP
...
- `#91` / `solaris.db` `voice_pe_rooms` table — the data plane this writes to.
...
- Longer term the room source of truth should be Home Assistant (device→area);
  this skill writes the interim `solaris.db` store. See `#94`.
