...
description: Use when a resident asks Solaris to find, search, recall, or look something up in the household notes / knowledge base / Obsidian vault (e.g. "what did we note about the garden?", "find the book I added about X", "wo hab ich das mit dem WLAN-Passwort notiert?", "such in meinen Notizen nach …"), or to show everything about a topic/project (e.g. "zeig mir alles zu Projekt Wintergarten", "was haben wir zu <Thema>", "show me everything about <topic>"). Retrieves matching notes from the Syncthing-synced vault under /opt/data/notes and feeds them back into the answer; a topic request filters by the `#topic/<slug>` tag. Read-only.
version: 1.1.0
author: Solaris
license: MIT
---
...
(`/opt/data/notes`, Syncthing-synced). This is the **read half** of Solaris's
knowledge base — the write half is `media-ingestion-multimodal` (book/album/
document notes), `solaris-dynamic-skills` (facts in `SOUL.md` / `fact_*.md`), and
`solaris-daily-chronicle` (journal). This skill lets the agent *find* and cite
what was written.
