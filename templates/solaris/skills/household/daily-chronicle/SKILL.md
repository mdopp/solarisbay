...
- A resident asks to create or update the household journal / chronicle /
  diary for a day:
  - "Solaris, schreib die Familienchronik für heute."
...
- Do **not** trigger on a request to write a *general* note or fact — that is
  the `media-ingestion-multimodal` / `solaris-dynamic-skills` path. This skill is
...
> - A thing a single resident told Solaris in private goes in the journal **only**
...
- **Don't self-schedule, don't restart services**: the daily cron is
  registered once by `solarisbay`'s post-deploy. This skill never creates
